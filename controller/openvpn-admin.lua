...
function action_get_hotplug_log()
    ...
    if sys.call("test -f " .. log_file .. " 2>/dev/null") == 0 then
        ...
        -- 获取文件大小
        local size_cmd = "stat -c%s " .. log_file .. " 2>/dev/null || echo 0"
        ...
        if result.log_size > 0 then
            ...
            if result.log_size > 102400 then
                result.log_content = sys.exec("tail -c 102400 " .. log_file .. " 2>/dev/null")
            else
                result.log_content = sys.exec("cat " .. log_file .. " 2>/dev/null")
            end
        end
